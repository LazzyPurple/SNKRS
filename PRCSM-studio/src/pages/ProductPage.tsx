...
export default function ProductPage() {
  ...
  const [selectedVariant, setSelectedVariant] =
    useState<NormalizedVariant | null>(null);
  ...
  const genderFilteredVariants = useMemo(() => {
    return normalizedVariants.filter(
      (variant) => variant.gender === selectedGender
    );
  ...
  const targetSizes = useMemo(() => {
    switch (selectedGender) {
      case "men":
        return MEN_SIZES.map(String);
      case "women":
        return WOMEN_SIZES.map(String);
      case "kids":
        return KIDS_SIZES;
      default:
        return [];
  ...
  const handleSizeSelect = (size: string) => {
    const variant = genderFilteredVariants.find(
  ...
  const isOutOfStock =
    genderFilteredVariants.length === 0 ||
    genderFilteredVariants.every((v) => !v.available);
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8 border-2 border-white inline-block px-4 py-2 shadow-[4px_4px_0px_0px_#A488EF]">
          <span className="font-bold text-lg">
            NIKE / {product.productType} / {product.title}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1">
              <div className="border-4 border-white shadow-[12px_12px_0px_0px_#A488EF] bg-prcsm-black">
                <img
                  src={
                    product.images.nodes[selectedImageIndex]?.url ||
                    product.images.nodes[0]?.url ||
                    "/placeholder-image.jpg"
                  }
                  alt={
                    product.images.nodes[selectedImageIndex]?.altText ||
                    product.title
                  }
          ...
          <div className="space-y-8 border-4 border-white p-6 shadow-[8px_8px_0px_0px_#A488EF]">
            ...
            <div className="space-y-3 border-t-4 border-white pt-4">
              ...
              >
                ...
                <SelectContent className="border-2 border-white bg-prcsm-black text-white [&_*]:bg-prcsm-black">
                  <SelectItem
                    value="men"
                    className="bg-prcsm-black hover:bg-gray-800 focus:bg-[#A488EF] focus:text-black"
                  >
                    Men
                  </SelectItem>
                  <SelectItem
            ...
            <div className="space-y-3 border-t-4 border-white pt-4">
              ...
              {isOutOfStock ? (
                <div className="text-red-400 font-bold border-2 border-red-400 p-2 inline-block">
                  OUT OF STOCK
                </div>
              ) : (
